...
int main()
{
	...
	long double Mk, C, f, p, q, S, Lp, Lq, V, E;
	long double Mr, A, B, H, s, n, N, T, Ttop, t, ss, Fm, Fn;
	...
	A = Mr * (1 – (q / 100)) * (S / 100); */
	A = Mr * (1 - (q / 100)) * (S / 100);
	printf("Şerit eksenleri arasındaki uzunluk (A) metre(m) değeri: %Lf\n", A);
	/* 2.3 Uçuş bazının hesaplanması
	B = Mr * (1 – (p / 100)) * (S / 100); */
	B = Mr * (1 - (p / 100)) * (S / 100);
	printf("Uçuş bazı (B) metre(m) değeri: %Lf\n", B);
	/* 2.4 Uçuş yüksekliğinin hesaplanması
 	H = (f / 100) * Mr; */
	H = (f / 100) * Mr;
	printf("Uçuş yüksekliği (H) metre(m) değeri: %Lf\n", H);
	/* 2.5 Resim kenarının yeryüzündeki karşılığının hesaplanması
 	s = (S / 100) * Mr; */
	s = (S / 100) * Mr;
	printf("Resim kenarının yeryüzündeki karşılığı (s) metre(m) değeri: %Lf\n", s);
	/* 2.6 Şerit sayısının hesaplanması
 	n = (((Lq * 1000) – s) / A) + 1; */
	n = (((Lq * 1000) - s) / A) + 1;
	printf("Şerit sayısı (n) adet değeri: %Lf\n", n);
	/* 2.7 Her bir şeritteki resim sayısının hesaplanması
 	N = ((Lp * 1000) / B) + 4; */
	N = ((Lp * 1000) / B) + 4;
	printf("Her bir şeritteki resim sayısı (N) adet değeri: %Lf\n", N);
	/* 2.8 Tüm Bloktaki resim sayısının hesaplanması
	T = n * N; */
	T = n * N;
	printf("Tüm bloktaki resim sayısı (T) adet değeri: %Lf\n", T);
	/* 2.9 Toplam uçuş süresinin hesaplanması
	Ttop = ((n * Lp) + (n – 1) * E) / V; */
	Ttop = ((n * Lp) + (n - 1) * E) / V;
	printf("Toplam uçuş süresi (Ttop) saniye(sn) değeri: %Lf\n", Ttop);
	/* 2.10 İki resim çekimi arasındaki sürenin hesaplanması (km/h'i m/sn çevirmek için 1000/3600 ile çarparız.)
 	t = (B / ((V * 1000 ) / 3600)); */
 	t = (B / ((V * 1000 ) / 3600));
	printf("İki resim çekimi arasındaki süre (t) saniye(sn) değeri: %Lf\n", t);
	/* 2.11 Yerde kaplanan alanın hesaplanması
 	ss = ((s / 1000) ^ 2); */
	ss = pow((s / 1000), 2);
	printf("Yerde kaplanan alan (ss) kilometre kare(km^2) değeri: %Lf\n", ss);
	/* 2.12 Bir modelin kapladığı alanın hesaplanması
 	Fm = (s – B) / 1000 * (s / 1000); */
 	Fm = (s - B) / 1000 * (s / 1000);
	printf("Bir modelin kapladığı alan (Fm) kilometre kare(km^2) değeri: %Lf\n", Fm);
	/* 2.13 Yeni kazanılan stereoskobik alanın hesaplanması
	Fn = (s / 1000) * (1 – (p / 100)) * (s / 1000) * (1 – (q / 100)); */
	Fn = (s / 1000) * (1 - (p / 100)) * (s / 1000) * (1 - (q / 100));
	printf("Yeni kazanılan stereoskobik alan (Fn) kilometre kare(km^2) değeri: %Lf\n", Fn);
